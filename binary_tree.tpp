...
BinaryTree<TreeItemType, FunctionType> BinaryTree<TreeItemType, FunctionType>::makeBinaryTree(const TreeItemType *array, int low, int high) {
    ...
    //create a left subtree using recursion
    BinaryTree left = makeBinaryTree(array, low, middle - 1);

    //create a right subtree using recursion
    BinaryTree right = makeBinaryTree(array, middle + 1, high);

    
    BinaryTree midlleTree(array[middle], left, right);
